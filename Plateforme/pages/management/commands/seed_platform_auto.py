...
class Command(BaseCommand):
    ...
    def _seed_specialties(self):
        ...
        for row in SPECIALTY_SEEDS:
            ...
            specialty = (
                Specialty.objects.filter(code=code).first()
                or Specialty.objects.filter(name_en=row["name_en"]).first()
            )
            if specialty is None:
                specialty = Specialty.objects.create(
                    code=code, name_en=row["name_en"], name_ar=row["name_ar"]
                )
            else:
                changed = False
                if specialty.code != code:
                    specialty.code = code
                    changed = True
                if specialty.name_en != row["name_en"]:
                    specialty.name_en = row["name_en"]
                    changed = True
                if specialty.name_ar != row["name_ar"]:
                    specialty.name_ar = row["name_ar"]
                    changed = True
                if changed:
                    specialty.save(update_fields=["code", "name_en", "name_ar"])
    ...
    def _seed_corpora(self, reset):
        corpora = []
        admin = self._get_or_create_admin()
        # Keep one corpus entry per base source (no News/Social/Scholarly/Dialect duplicates).
        for base in CORPUS_BASES:
            title = base["name"]

            # Cleanup legacy seeded variants so the section shows one container only.
            for variant in CORPUS_VARIANTS:
                legacy_title = f"{base['name']} {variant['suffix']}"
                if legacy_title != title:
                    Corpus.objects.filter(title=legacy_title).delete()
            ...
            obj, created = Corpus.objects.get_or_create(
                ...
                defaults={
                    "title_ar": base["name"],
                    "title_en": title,
                    "description": base["description_ar"],
                    "description_ar": base["description_ar"],
            ...
            obj.title_ar = base["name"]
            obj.title_en = title
            obj.description = base["description_ar"]
            obj.description_ar = base["description_ar"]
            obj.description_en = base["description_en"]
            obj.author = admin
