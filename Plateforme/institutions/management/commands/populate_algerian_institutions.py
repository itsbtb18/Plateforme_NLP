...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        algeria, _ = Country.objects.get_or_create(
            code="DZ", defaults={"name_en": "Algeria", "name_ar": "الجزائر"}
        )
        ...
        for inst_data in institutions:
            ...
            matches = Institution.objects.filter(acronym=acronym).order_by("id")
            if matches.exists():
                institution = matches.first()
                created = False
                duplicate_count = max(matches.count() - 1, 0)
                if duplicate_count:
                    self.stdout.write(
                        self.style.WARNING(
                            f"! Found {duplicate_count} duplicate record(s) for acronym {acronym}; updating the first one."
                        )
                    )
            else:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Created: {institution.acronym} - {institution.name}"
                    )
                )
                created_count += 1
            else:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"⟳ Updated: {institution.acronym} - {institution.name}"
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Total institutions in database: {Institution.objects.count()}"
            )
        )
