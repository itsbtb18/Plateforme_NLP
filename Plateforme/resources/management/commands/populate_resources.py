...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not author:
            self.stdout.write(
                self.style.WARNING("No superuser found to assign as author")
            )
            return
        ...
        for corpus_data in corpora_data:
            if not Corpus.objects.filter(title_en=corpus_data["title_en"]).exists():
                ...
                corpus_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Created corpus: {corpus_data['title_en']}")
                )
        ...
        for tool_data in tools_data:
            if not NLPTool.objects.filter(title_en=tool_data["title_en"]).exists():
                ...
                tool_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Created tool: {tool_data['title_en']}")
                )
        ...
        for doc_data in documents_data:
            if not Document.objects.filter(title_en=doc_data["title_en"]).exists():
                ...
                doc_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Created document: {doc_data['title_en']}")
                )
        ...
        if not institution:
            self.stdout.write(
                self.style.WARNING("No institution found. Skipping courses.")
            )
        else:
            courses_data = [
                {
            ...
            for course_data in courses_data:
                if not Course.objects.filter(title_en=course_data["title_en"]).exists():
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Created course: {course_data['title_en']}"
                        )
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Corpora: {corpus_count} new, Total: {Corpus.objects.count()}"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"NLP Tools: {tool_count} new, Total: {NLPTool.objects.count()}"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Documents: {doc_count} new, Total: {Document.objects.count()}"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Courses: {course_count} new, Total: {Course.objects.count()}"
            )
        )
