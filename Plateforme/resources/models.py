...
class ResourceBase(models.Model):
    ...
    rejected_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="rejected_%(app_label)s_%(class)s_set",
    )
    ...
    def get_supported_languages_list(self):
        ...
        if supported_langs:
            return [
                lang.strip() for lang in str(supported_langs).split(",") if lang.strip()
            ]
        return []
...
def validate_academic_year(value):
    ...
    if end_year != start_year + 1:
        raise ValidationError(_("The ending year must be the starting year + 1"))
    return value
...
class Thesis(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    supervisor = models.CharField(max_length=100, verbose_name=_("Thesis Supervisor"))
    institution = models.ForeignKey(
        Institution, on_delete=models.PROTECT, verbose_name=_("Institution")
    )
    defense_year = models.IntegerField(verbose_name=_("Defense Year"))
...
class Article(models.Model):
    ...
    journal = models.CharField(max_length=200, verbose_name=_("Journal"))
    ...
    def get_citation(self):
        year = (
            self.publication_date.year if self.publication_date else _("Unknown year")
        )
...
class NLPTool(ResourceBase):
    ...
    def clean(self):
        ...
        if (
            self.tool_type == self.ToolType.MACHINE_TRANSLATION
            and not (self.supported_languages or "").strip()
        ):
            raise ValidationError(
    ...
    def get_supported_languages_list(self):
        ...
        if self.supported_languages:
            return [
                lang.strip()
                for lang in str(self.supported_languages).split(",")
                if lang.strip()
            ]
        return []
...
class Corpus(ResourceBase):
    ...
    def get_absolute_url(self):
        return reverse("resources:corpus_detail", kwargs={"pk": self.pk})
...
def index_resource(sender, instance, **kwargs):
    try:
        ...
        logger.warning(
            ...
            "Skipping search indexing for this resource.",
            instance.title,
            exc,
        )
