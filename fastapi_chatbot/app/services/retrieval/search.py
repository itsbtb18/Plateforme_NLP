...
from app.models import (
    PlatformDoc, NLPKnowledge, Resource,
...
async def search_platform_docs(
    query: str, db: AsyncSession, top_k: Optional[int] = None,
) -> List[Dict]:
    try:
...
async def search_nlp_knowledge(
    ...
    top_k: Optional[int] = None, language: Optional[str] = None,
) -> List[Dict]:
    try:
        k = top_k or settings.TOP_K_RESULTS
        embedding_svc = get_embedding_service()
        ...
        hits = qdrant.search(
            ...
            query_vector=qe, limit=k, query_filter=qf,
        )
        if not hits:
            return []
        ...
        for pid in ids:
            ...
            if not r:
                continue
            results.append({
                "id": r.id, "title": r.topic, "content": r.content,
                "language": r.language, "keywords": r.keywords,
                "difficulty": r.difficulty, "source": "nlp_knowledge",
                "similarity": score_map[pid],
            })
        return results
    except Exception as e:
...
async def search_resources(
    ...
    try:
        ...
        hits = qdrant.search(
            ...
            query_vector=qe, limit=k * 2, query_filter=qf,
        )
        if not hits:
            return []
        ...
        for pid in ids:
            ...
            results.append({
                ...
                "content": r.description, "url": r.url, "tags": r.tags,
                "country": r.country, "city": r.city, "author": r.author,
                "institution": r.institution, "year": r.year,
                "source": "resources", "similarity": sim,
            })
    ...
    try:
        ...
        if language:
            ...
            lang_hits = qdrant.search(
                ...
                query_vector=qe, limit=k, query_filter=lang_filter,
            )
            if len(lang_hits) >= max(1, k // 2):
                ...
                all_hits = qdrant.search(
                    collection=COLLECTION_LEGAL_DOCUMENTS,
                    query_vector=qe, limit=k, query_filter=base_filter,
                )
        ...
        for pid in ids:
            ...
            results.append({
                ...
                "keywords": r.keywords, "source": "legal",
                "similarity": sim,
            })
        return results
    except Exception as e:
