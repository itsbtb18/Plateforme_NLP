...
async def cleanup_nlp(
    ...
    async with AsyncSessionLocal() as db:
        ...
        logger.info("Current state: %d total rows (%d seed, %d PDF chunks)", total, seed_count, pdf_count)
        ...
        if mode == "all":
            ...
            logger.info("Mode: PDF-ONLY — will delete %d PDF chunk rows (keeping %d seeds)", target_count, seed_count)
        ...
        if mode == "all":
            ...
            await db.execute(
                delete(NLPKnowledge).where(NLPKnowledge.id > SEED_MAX_ID)
            )
        await db.commit()
        ...
        if reset_sequence:
            if mode == "all":
                await db.execute(text("ALTER SEQUENCE nlp_knowledge_id_seq RESTART WITH 1"))
            else:
                next_val = SEED_MAX_ID + 1
                await db.execute(text(f"ALTER SEQUENCE nlp_knowledge_id_seq RESTART WITH {next_val}"))
            await db.commit()
...
if __name__ == "__main__":
    ...
    group.add_argument("--pdf-only", action="store_true", help="Delete only PDF chunks, keep seeds")
    parser.add_argument("--dry-run", action="store_true", help="Preview only, no changes")
    parser.add_argument("--reset-sequence", action="store_true", help="Reset PostgreSQL ID sequence")
    args = parser.parse_args()

    mode = "all" if args.all else "pdf-only"
    asyncio.run(cleanup_nlp(mode=mode, dry_run=args.dry_run, reset_sequence=args.reset_sequence))
