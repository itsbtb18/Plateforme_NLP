...
from app.services.qdrant import get_qdrant_service, COLLECTION_NLP_KNOWLEDGE, COLLECTION_LEGAL_DOCUMENTS
...
async def process_file(file_path: Path, doc_type: str, processor, embedding_service, db, qdrant):
    ...
    try:
        ...
        chunk_texts = [c["content"].strip() for c in chunks if c["content"].strip()]
        if not chunk_texts:
            return

        # Detect language from first chunk (reliable for whole-document language)
        file_language = _detect_language(chunk_texts[0])
        logger.info("  Detected language: %s, %d chunks to embed", file_language, len(chunk_texts))

        # --- PC-safe: embed in tiny batches with sleep ---
        points: List[PointStruct] = []
        collection = COLLECTION_LEGAL_DOCUMENTS if doc_type == "law" else COLLECTION_NLP_KNOWLEDGE
        ...
        for i in range(0, len(chunk_texts), EMBED_BATCH_SIZE):
            ...
            for idx_in_batch, (content, embedding) in enumerate(zip(batch_texts, embeddings)):
                ...
                if doc_type == "law":
                    ...
                    payload = {
                        ...
                        "article_heading": batch_chunks[idx_in_batch].get("article_heading"),
                    }
                else:
            ...
            logger.info("  Embedded %d/%d chunks", min(i + EMBED_BATCH_SIZE, len(chunk_texts)), len(chunk_texts))
        ...
        await db.commit()
        logger.info("✅ Indexed %d chunks from %s [lang=%s]", len(chunk_texts), file_path.name, file_language)

        # Free memory
        gc.collect()

    except Exception as e:
        logger.error("❌ Failed to process %s: %s", file_path.name, str(e), exc_info=True)
        await db.rollback()


async def main():
    parser = argparse.ArgumentParser(description="Ingest a folder of documents.")
    parser.add_argument("--folder", type=str, required=True, help="Path to folder to scan")
    parser.add_argument("--type", type=str, choices=["nlp", "law"], default="nlp", help="Type of knowledge")
    ...
    async with AsyncSessionLocal() as db:
        for file_idx, file_path in enumerate(files):
            ...
            await process_file(file_path, args.type, processor, embedding_service, db, qdrant)
