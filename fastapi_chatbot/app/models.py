...
class UserDocument(Base):
    ...
    # Feature 2: distinguish platform documents from user uploads
    source = Column(String(50), default="user_upload")  # user_upload | platform
    platform_document_id = Column(String(100))  # Django resource PK/UUID (if source=platform)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
