...
\begin{table}[H]
\centering
\caption{Overall automated evaluation metrics for the Retrieval-Augmented Generation pipeline.}
\label{tab:eval_results}
\small
\begin{tabularx}{\textwidth}{L{4cm} C{2cm} C{2cm} C{2cm} C{3cm}}
\toprule
\rowcolor{tablehead}\color{white}\textbf{Configuration} & \color{white}\textbf{P@5} & \color{white}\textbf{R@5} & \color{white}\textbf{MRR} & \color{white}\textbf{BERTScore F1} \\
\midrule
\texttt{Production RAG System} & 0.7524 & 0.9405 & 0.8786 & 0.9472 \\
\bottomrule
\end{tabularx}
\end{table}

\textit{Interpretation:} A BERTScore F1 of 0.9472 indicates very strong semantic alignment between generated answers and reference answers, even when phrasing differs.

\textbf{Key observations:}
\begin{itemize}[leftmargin=2em]
  \item The system achieves a high \textbf{Precision@5 (0.7524)} and excellent \textbf{Recall@5 (0.9405)}, indicating that the Reciprocal Rank Fusion (BM25 + BGE-M3) and contextual reranking effectively bubble the most relevant passages to the top of the context window.
  \item The \textbf{Mean Reciprocal Rank (0.8786)} demonstrates that the very first retrieved document is highly likely to be strongly relevant to the user's query.
      \item In the main benchmark configuration, the pipeline exhibited \textbf{0 failure cases}.
  \item The overall Generation quality recorded a \textbf{BERTScore Precision of 0.9710}, a \textbf{Recall of 0.9250}, and an \textbf{F1 of 0.9472}. This implies extremely high syntactic and structural semantic similarity to the reference truth, with an exceedingly low rate of hallucination or off-topic generation.
      \item Different JSON versions (including smaller datasets) contain different values; benchmark interpretation is therefore tied to explicit scenario and dataset size.
\end{itemize}
...
When the Web Fallback was disabled, the system safely and correctly yielded \textbf{zero relevant context} for the pure OOD queries like "Nanochat". Because the information simply did not exist in the local corpus, the LLM was unable to answer the questions, leading to generation failures or polite refusals. This proves that while the baseline RAG is safe and resists hallucination, it is rigidly confined by its static knowledge base.

However, upon reactivating the Web Fallback mechanism, the Intent Router successfully detected the lack of local context and dynamically triggered the Exa API. For the exact same queries:
\begin{itemize}[leftmargin=2em]
    \item The system successfully retrieved highly relevant external context (e.g., GitHub repositories and tech blogs for "Nanochat").
    \item The system achieved high retrieval precision by successfully surfacing the required external information.
    \item The generated answers were highly accurate, well-cited, and achieved a high \textbf{BERTScore F1} consistent with the overall system average.
\end{itemize}

This experimentation definitively confirms that the Web Fallback is an essential architectural component. It acts as a critical safety net that prevents catastrophic retrieval failures when users inevitably pose questions outside the system's core corpus, transforming a rigid local RAG into a dynamic, agentic research assistant.
